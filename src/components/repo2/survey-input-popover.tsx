  ...
  const [matrixColumnInput, setMatrixColumnInput] = useState<string[]>([]);
    ...
    function addField(type:FieldTypes){
      ...
      if(type==FieldTypes.DATE){
        setFormFields([...formFields,{"type":FieldTypes.DATE,"label":labelValue}])
        // setLabelValue('');
      }

      if(type==FieldTypes.MATRIX){
        setFormFields([...formFields, {"type":FieldTypes.MATRIX,"label":labelValue,"matrixRow":matrixRowInput, "matrixColumn":matrixColumnInput}])
        // setMultipleInput([]);
        // setLabelValue('');
      }
        ...
        {/* <form action={addField}> */}
            ...
            <div className="space-y-2">
                ...
                {fieldType==FieldTypes.DATE &&
                  ...
                  <Button className='' onClick={()=>addField(FieldTypes.DATE)}>Submit Choices</Button>
                  </div>
                }
                  ...
                  {fieldType==FieldTypes.MATRIX &&
                   ...
                   <Input
                       ...
                       defaultValue="Your Label"
                       className="col-span-2 h-8"
                       onChange={(e)=>{setInputValueSecond(e.target.value);console.log(e.target.value)}}
                   />

                   <button onClick={()=>setMatrixRowInput([...matrixRowInput, inputValue])}>Add Row</button>

                   <button onClick={()=>setMatrixColumnInput([...matrixColumnInput, inputValueSecond])}>Add Column</button>
                   ...
                   <Button className='' onClick={()=>addField(FieldTypes.MATRIX)}>Submit Choices</Button>
                   </div>
                }
